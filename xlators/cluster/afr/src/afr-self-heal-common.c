                         ...
                         unsigned char *sources, void *gfid, int *gfid_idx)
{
    ...
    if (source >= 0 && replies[source].valid && IS_SUCCESS(replies[source].op_ret))
    ...
    for (i = 0; i < priv->child_count; i++) {
        if (source == -1) {
            ...
            if (i == source)
                continue;
            if (sources[i] && replies[i].valid && IS_SUCCESS(replies[i].op_ret) &&
                replies[i].poststat.ia_type != IA_INVAL) {
                ia_type = replies[i].poststat.ia_type;
                break;
            }
                               ...
                               int *lock_count, int *eagain_count)
{
    ...
    for (i = 0; i < priv->child_count; i++) {
        ...
        if (IS_SUCCESS(replies[i].op_ret)) {
            (*lock_count)++;
        } else if (IS_ERROR(replies[i].op_ret) && replies[i].op_errno == EAGAIN) {
            (*eagain_count)++;
        }
    }
}
