...
ec_manager_entrylk(ec_fop_data_t *fop, int32_t state)
{
    ...
    switch (state) {
        ...
        case -EC_STATE_REPORT:
            ...
            if (fop->id == GF_FOP_ENTRYLK) {
                if (fop->cbks.entrylk != NULL) {
                    fop->cbks.entrylk(fop->req_frame, fop, fop->xl, gf_failure,
                                      fop->error, NULL);
                }
            } else {
                if (fop->cbks.fentrylk != NULL) {
                    fop->cbks.fentrylk(fop->req_frame, fop, fop->xl, gf_failure,
                                       fop->error, NULL);
                }
            }
...
ec_manager_inodelk(ec_fop_data_t *fop, int32_t state)
{
    ...
    switch (state) {
        ...
        case -EC_STATE_REPORT:
            ...
            if (fop->id == GF_FOP_INODELK) {
                if (fop->cbks.inodelk != NULL) {
                    fop->cbks.inodelk(fop->req_frame, fop, fop->xl, gf_failure,
                                      fop->error, NULL);
                }
            } else {
                if (fop->cbks.finodelk != NULL) {
                    fop->cbks.finodelk(fop->req_frame, fop, fop->xl, gf_failure,
                                       fop->error, NULL);
                }
            }
...
ec_manager_lk(ec_fop_data_t *fop, int32_t state)
{
    ...
    switch (state) {
        ...
        case -EC_STATE_REPORT:
            ...
            if (fop->cbks.lk != NULL) {
                fop->cbks.lk(fop->req_frame, fop, fop->xl, gf_failure, fop->error, NULL,
                             NULL);
            }
