                   ...
                   struct iatt *postbuf, dict_t *xdata)
{
    ...
    ec_trace("WRITE_CBK", cookie, "ret=%d, errno=%d", GET_RET(op_ret), op_errno);

    gf_msg_debug(fop->xl->name, 0,
                 "%s: write op_ret %d, op_errno %s"
                 " at %" PRIu64,
                 uuid_utoa(heal->fd->inode->gfid), GET_RET(op_ret), strerror(op_errno),
                 heal->offset);
                            ...
                            gf_boolean_t check_ondisksize, int which)
{
    ...
    if (max_same_count < ec->fragments) {
        ...
        for (i = 0; i < ec->nodes; i++) {
	  if (replies[i].valid && IS_SUCCESS(replies[i].op_ret) && !sources[i])
                healed_sinks[i] = 1;
        }
    }
...
ec_manager_heal_block(ec_fop_data_t *fop, int32_t state)
{
    ...
    switch (state) {
        ...
        case EC_STATE_REPORT:
            if (fop->cbks.heal) {
                fop->cbks.heal(fop->req_frame, fop->data, fop->xl, gf_zero_ret, 0,
        ...
        case -EC_STATE_REPORT:
            if (fop->cbks.heal) {
                fop->cbks.heal(fop->req_frame, fop->data, fop->xl, gf_failure,
...
out:
    if (fop != NULL) {
        ...
        func(frame, heal, this, gf_failure, error, 0, 0, 0, 0, NULL);
    }
}
...
{
    if (fop->cbks.heal) {
        fop->cbks.heal(fop->req_frame, fop->data, ec->xl, gf_failure, fop->error, 0, 0,
                       0, 0, NULL);
    }
...
fail:
    if (fop)
        ec_fop_data_release(fop);
    if (func)
        func(frame, data, this, gf_failure, err, 0, 0, 0, 0, NULL);
}
