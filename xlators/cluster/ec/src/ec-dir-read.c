...
ec_manager_opendir(ec_fop_data_t *fop, int32_t state)
{
    ...
    switch (state) {
        ...
        case -EC_STATE_REPORT:
            ...
            if (fop->cbks.opendir != NULL) {
                fop->cbks.opendir(fop->req_frame, fop, fop->xl, gf_failure, fop->error,
                                  NULL, NULL);
            }
...
ec_manager_readdir(ec_fop_data_t *fop, int32_t state)
{
    ...
    switch (state) {
        ...
        case -EC_STATE_REPORT:
            if (fop->id == GF_FOP_READDIR) {
                if (fop->cbks.readdir != NULL) {
                    fop->cbks.readdir(fop->req_frame, fop, fop->xl, gf_failure,
                                      fop->error, NULL, NULL);
                }
            } else {
                if (fop->cbks.readdirp != NULL) {
                    fop->cbks.readdirp(fop->req_frame, fop, fop->xl, gf_failure,
                                       fop->error, NULL, NULL);
                }
            }
            if (fop->offset == 0)
