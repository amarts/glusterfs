...
meta_default_opendir(call_frame_t *frame, xlator_t *this, loc_t *loc, fd_t *fd,
                     dict_t *xdata)
{
    META_STACK_UNWIND(opendir, frame, gf_zero_ret, 0, fd, xdata);
    return 0;
}
...
meta_default_fstat(call_frame_t *frame, xlator_t *this, fd_t *fd, dict_t *xdata)
{
    ...
    META_STACK_UNWIND(fstat, frame, gf_zero_ret, 0, &iatt, xdata);
...
{
    META_STACK_UNWIND(flush, frame, gf_zero_ret, 0, xdata);
    return 0;
}
                    ...
                    uint32_t flags, struct iobref *iobref, dict_t *xdata)
{
    ...
    SET_RET(op_ret, ret);
    META_STACK_UNWIND(writev, frame, (IS_SUCCESS(op_ret) ? op_ret : gf_failure),
                  ...
                  int32_t flags, fd_t *fd, dict_t *xdata)
{
    ...
    META_STACK_UNWIND(open, frame, gf_zero_ret, 0, fd, xdata_rsp);
                      ...
                      size_t size, dict_t *xdata)
{
	...
	SET_RET(op_ret, len);
        META_STACK_UNWIND(readlink, frame, op_ret, 0, strfd->data, &iatt, xdata);
    } else
                       ...
                       off_t offset, dict_t *xdata)
{
    ...
    META_STACK_UNWIND(ftruncate, frame, gf_zero_ret, 0, &iatt, &iatt, xdata);
                      ...
                      off_t offset, dict_t *xdata)
{
    ...
    META_STACK_UNWIND(truncate, frame, gf_zero_ret, 0, &iatt, &iatt, xdata);
                  ...
                  dict_t *xdata)
{
    ...
    META_STACK_UNWIND(stat, frame, gf_zero_ret, 0, &iatt, xdata);
...
hook:
    if (dirent && dirent->hook) {
        ...
        META_STACK_UNWIND(lookup, frame, gf_zero_ret, 0, loc->inode, &iatt, xdata,
                          &parent);
    } else {
